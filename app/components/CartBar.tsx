...
export default function CartBar({ cart, onViewCart }: Props) {
  ...
  return (
    <div className="w-full bg-accent-300 shadow px-6 py-4 grid grid-cols-4 items-center justify-between absolute left-0 bottom-0 z-10 gap-2">
      ...
      <button className="text-accent-300 col-span-3 bg-white text-lm font-bold rounded-[16px] px-4 h-10 flex items-center gap-2 justify-center" onClick={onViewCart}>
        Ver carrito
      </button>
    </div>
  );
